# Latest Scraper Results

## 2026-03-21 04:28:28

### [https://www.drw.com/work-at-drw/listings?filterType=keyword&value=software](https://www.drw.com/work-at-drw/listings?filterType=keyword&value=software)

**Line changes detected!**

```diff
No line changes.
```

---
### [https://joinbytedance.com/search?job_category_id_list=&location_code_list=CT_163&recruitment_id_list=202%2C301%2C201&subject_id_list=&tag_id_list=&keyword=Engineer&limit=12&offset=0](https://joinbytedance.com/search?job_category_id_list=&location_code_list=CT_163&recruitment_id_list=202%2C301%2C201&subject_id_list=&tag_id_list=&keyword=Engineer&limit=12&offset=0)

**Line changes detected!**

```diff
No line changes.
```

---
### [https://www.linkedin.com/jobs/search/?currentJobId=4296859263&f_C=6575553&f_CR=102454443&f_TPR=r86400&geoId=92000000&origin=JOB_SEARCH_PAGE_JOB_FILTER&sortBy=R&spellCorrectionEnabled=true&trk=d_flagship3_company](https://www.linkedin.com/jobs/search/?currentJobId=4296859263&f_C=6575553&f_CR=102454443&f_TPR=r86400&geoId=92000000&origin=JOB_SEARCH_PAGE_JOB_FILTER&sortBy=R&spellCorrectionEnabled=true&trk=d_flagship3_company)

**Line changes detected!**

```diff
- ,
- , and
- .
+ 1 day ago
+ 1 hour ago
+ 14 hours ago
+ 15 hours ago x2
+ 17 hours ago
+ 18 hours ago x16
+ 48
+ 48 jobs in Worldwide
+ 7 hours ago x3
+ 8 hours ago x23
+ AI Product Solution Architect, BytePlus - London x2
+ AI Product Solutions Architect (Mandarin Speaking) - BytePlus x2
+ AI Technical Support Specialist x2
+ Account Executive - Innovative Technical Sales, BytePlus (GenAI & SaaS) x2
+ Account Manager - Lark - Indonesia x2
+ Actively Hiring x16
- Agree & Join
+ Allied Universal (881)
- Already on Linkedin?
+ Any time (1,062)
+ Associate (1)
+ Backend Software Engineer (Cloud Platform) - Cloud Infrastructure x2
+ Backend Software Engineer Intern (Service Platform) - 2026 Start (BS/MS) x2
+ Bangkok, Bangkok City, Thailand x3
+ Be an early applicant x32
+ Beijing, Beijing, China
- By clicking Agree & Join, you agree to the LinkedIn
+ ByteDance x49
+ ByteDance (48)
+ BytePlus Account Executive – E-commerce ISV (GenAI & SaaS) x2
+ Capgemini (847)
+ Cashloan Product Manager - Global Payment x2
+ Clear text x6
+ Cloud Native Observability Management Platform Engineer - Data Infrastructure x2
+ Cloud Optical Transport Network (OTN) Engineer - Physical Network Infrastructure x2
+ Cloud Sourcing Specialist x2
- Continue with Google x2
- Cookie Policy
+ Credit Loan Collection Manager - Global Payment (Malaysia) x2
+ Credit Loan Collection Manager - Global Payment - Indonesia x2
+ Customer Success Manager - Lark - Jakarta x2
+ Customer Success Manager Project Intern (Lark) - 2026 Start (BS/MS) x2
+ Customer Success Manager, Lark - Thailand x2
+ Data Center Technical Project Manager, Data Center Development x2
+ Done x5
+ Dubai, Dubai, United Arab Emirates x4
- Email
+ Expand search
+ Experience level
- First name
+ Full-time (37)
+ General Procurement Specialist - Catering Services x2
+ Global Partnership & Ecosystem Manager - Byteplus x2
+ Gurgaon, Haryana, India
+ HR Business Partner - Global E-Commerce Operations - Taguig x2
+ HR Business Partner, Project Intern (Data) - 2026 Start (BS/MS) x2
+ HRBP Project Intern (Games) - 2026 Start (BS/MS) x2
+ Hong Kong SAR x3
+ Hybrid (9)
+ Innovation Tech Solution Sales - BytePlus - Gurgaon x2
+ Innovation Tech Solution Sales Specialist - BytePlus x2
+ International Tax Business Partner - Byteplus, Lark, Capcut (Tax BP-Global) x2
+ Internship (11)
+ Internship (17)
+ Jakarta, Jakarta, Indonesia x3
+ Job type
+ Jobs
+ Jobs in Worldwide
- Join LinkedIn
+ Join now
+ Kuala Lumpur, Federal Territory of Kuala Lumpur, Malaysia x2
+ Kulai, Johore, Malaysia
- Last name
+ Learning
+ Legal Counsel/ Senior Legal Counsel (Payments), Dubai x2
+ London, England, United Kingdom x2
+ Los Angeles, CA
+ Machine Learning Engineer Graduate (TikTok E-Commerce - Conversational AI)-2026 Start (BS/MS) x2
+ Machine Learning Engineer Graduate (TikTok E-Commerce - Conversational AI)-2026 Start (PhD) x2
+ Machine Learning Engineer Intern (Global E-commerce Risk Control) - 2026 Summer (BS/MS) x2
+ Machine Learning Graduate (eCommerce Recommendation) - 2026 Start (BS/MS) x2
+ Machine Learning Graduate (eCommerce User Growth & Intelligent Marketing) - 2026 Start (PhD) x2
+ Mid-Senior level (29)
+ Network Engineer, High Performance GPU Network Direction - San Jose x2
+ Network Implementation Engineer - Physical Network Infrastructure x2
+ New to LinkedIn?
- New to Linkedin?
- Not you?
+ On-site (39)
- Password (6+ characters)
+ Past 24 hours
+ Past 24 hours (48)
+ Past month (874)
+ Past week (198)
+ People
- Privacy Policy
+ Remote
- Remove photo
+ Research Scientist Project Intern (Multimodal Interaction & World Model) - 2026 Start (PhD) x2
+ Reset
+ San Jose, CA x11
+ Seattle, WA x4
- Security verification
+ Senior Tax Planning Manager - LATAM x2
+ Senior Transfer Pricing Manager - AMS x2
- Sign Up | LinkedIn
- Sign in
+ Sign in to view more jobs
+ Sign in with Email
+ Singapore x4
+ Singapore, Singapore x6
+ Software Development Engineer in Test Intern (Global E-Commerce) - 2026 Summer (BS/MS) x2
+ Software Engineer Graduate (AI Platform)- 2026 Start (PhD) x2
+ Software Engineer Graduate (Cloud Native Infrastructure)- 2026 Start (BS/MS) x2
+ Software Engineer Intern (Developer Infrastructure) - 2026 Summer (BS/MS) x2
+ Software Engineer Intern (ML System) - 2026 Start (PhD) x2
+ Software Engineer Intern(Media Network Global Engineering) - 2026 Summer (BS/MS) x2
+ Software Engineer Project Intern (Network Security) - 2026 Start (BS/MS) x2
+ Solutions Architect - BytePlus - Hong Kong x2
+ São Paulo, São Paulo, Brazil
+ Taguig, National Capital Region, Philippines
+ Talent Acquisition Partner - Data - Singapore x2
+ Technical Project Manager - Data Center Development x2
+ This button displays the currently selected search type. When expanded it provides a list of search options that will switch the search inputs to match the current selection.
- User Agreement
+ Walmart (2,900)
+ Worldwide
+ You've viewed all jobs for this search
+ myGwork - LGBTQ+ Business Community (639)
- or
+ 整合营销经理（新媒体方向）-即梦（北京/深圳） x2
```

---
