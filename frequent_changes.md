# Latest Frequent Scraper Results

## 2026-04-23 19:03:57

### [https://www.linkedin.com/jobs/search/?currentJobId=4296859263&f_C=6575553&f_CR=102454443&f_TPR=r21600&geoId=92000000&origin=JOB_SEARCH_PAGE_JOB_FILTER&sortBy=R&spellCorrectionEnabled=true&trk=d_flagship3_company](https://www.linkedin.com/jobs/search/?currentJobId=4296859263&f_C=6575553&f_CR=102454443&f_TPR=r21600&geoId=92000000&origin=JOB_SEARCH_PAGE_JOB_FILTER&sortBy=R&spellCorrectionEnabled=true&trk=d_flagship3_company)

**Line changes detected!**

```diff
- ,
- , and
- .
+ 13 minutes ago x2
+ 15 minutes ago
+ 18 minutes ago
+ 21 minutes ago x3
+ 22
+ 22 jobs in Worldwide
+ 25 minutes ago
+ 28 minutes ago x4
+ 30 minutes ago
+ 32 minutes ago x2
+ 33 minutes ago
+ 35 minutes ago
+ 39 minutes ago
+ 4 minutes ago
+ 40 minutes ago
+ 6 minutes ago
+ 8 minutes ago
+ AI Product Manager (Evaluation) - Flow x2
+ AI and Cloud Solution Architect, BytePlus - Ho Chi Minh x2
+ Account Executive - SaaS Commercial - Lark Japan x2
+ Advance Auto Parts (1,090)
- Agree & Join
- Already on Linkedin?
+ Any time (1,179)
+ Associate (2)
+ BNPL Solution Product manager - Global Payment x2
+ Be an early applicant x22
+ Beijing, Beijing, China
- By clicking Agree & Join, you agree to the LinkedIn
+ ByteDance x23
+ ByteDance (22)
+ Chief Information Security Officer - Cloud Security - Saudi Arabia x2
+ Clear text x6
+ Cloud Security Architect, ByteDance x2
+ Collections Manager - Global Payment x2
- Continue with Google x2
- Cookie Policy
+ Corporate Governance Manager - Global Payment x2
+ Country Director, Financial Services - Global Payment (Malaysia) x2
+ Country Director, Financial Services - Global Payment (Vietnam) x2
+ Deloitte (985)
+ Director (1)
+ Done x5
- Email
+ Employee Relations Partner - APAC x2
+ Executive (7)
+ Expand search
+ Experience level
- First name
+ Full-time (22)
+ Ho Chi Minh City, Vietnam x2
+ Hybrid (5)
+ Integrated Marketing Operation Manager - Global Payment x2
+ Jakarta, Jakarta, Indonesia
+ Job type
+ Jobs
+ Jobs in Worldwide
- Join LinkedIn
+ Join now
+ Kuala Lumpur, Federal Territory of Kuala Lumpur, Malaysia x2
- Last name
+ Learning
+ Linux System Engineer - Systems Technologies and Engineering - London x2
+ London, England, United Kingdom
+ Los Angeles, CA
+ Mid-Senior level (12)
+ New to LinkedIn?
- New to Linkedin?
- Not you?
+ On-site (15)
+ Panda Restaurant Group (1,325)
- Password (6+ characters)
+ Past 24 hours
+ Past 24 hours (22)
+ Past 24 hours (67)
+ Past month (986)
+ Past week (276)
+ People
- Privacy Policy
+ PwC (719)
+ Remote
- Remove photo
+ Reset
+ Riyadh, Riyadh, Saudi Arabia x3
+ San Jose, CA x2
- Security verification
+ Senior Lending Manager - Global Payment - Indonesia x2
+ Senior Production System Engineer - San Jose x2
+ Shanghai, Shanghai, China x3
- Sign Up | LinkedIn
- Sign in
+ Sign in to view more jobs
+ Sign in with Email
+ Singapore, Singapore x4
+ São Paulo, São Paulo, Brazil
+ Tax Director - Americas (Los Angeles) x2
+ Tax Director - Americas (San Jose) x2
+ Technical Project Manager - BytePlus (GenAI & SaaS) x2
+ This button displays the currently selected search type. When expanded it provides a list of search options that will switch the search inputs to match the current selection.
+ Tokyo, Tokyo, Japan
- User Agreement
+ Worldwide
+ You've viewed all jobs for this search
- or
+ 产品内容设计师-TikTok直播（上海） Content Designer-TikTok LIVE(Shanghai) x2
+ 本地化经理-TikTok直播（上海）Localization Manager-TikTok LIVE (Shanghai) x2
+ 本地化经理-TikTok直播（北京）Localization Manager-TikTok LIVE (Beijing) x2
+ 项目经理（治理与体验）-TikTok Shop x2
```

---
